...
var hasardFacile = arrayFacile[Math.floor(Math.random()*arrayFacile.length)];
...
let length = hasardFacile.length
...
//fonction pour avoir un indice
function getIndice() {
  //définie indiceFacile comme un string vide
  var indiceFacile = "";

  //for loop pour choisir une lettre au hasard dans le mot choisi
  for (var i = 0; i < 1; i++) 
    indiceFacile += hasardFacile.charAt(Math.floor(Math.random() * hasardFacile.length));
  //affiche l'indice sur l'écran
  var afficheIndiceFacile = ("Indice: " + indiceFacile);
  indice.innerHTML = afficheIndiceFacile
}
...
function jeuPerdu() {
  ...
  if (erreures == maxErreures) {
    ...
    motFin.innerHTML = 'Le mot était ' + hasardFacile
  }
}
...
function clavier(lettreChoisi) {
  ...
  for (let i = 0; i < etatMot.length; i++) {
    ...
    if (hasardFacile[i] == lettreChoisi ) {
